...
class MovieLensConfig(ConfigBase):
    experiment_name: str = "ml1m-new"
    dataset_name: str = "movie-lens-new-seqset"
    emb_dim: int = 16
...
class SpecialModel(BaseModel):
    def __init__(self, schema_manager: SchemaManager):
        ...
        self.real_time = RelativeTimeEmbedding(embedding_dim=seq_dim)
    ...
    def concat_embed_input_fields(self, interaction):
        ...
        seq_emb, seq_tar, seq_len, hist_timestamp = self.omni_embedding.seq_groups["history"].fetch_all(
            interaction,
            include_time=True,
        )
        curr_ts = interaction[self.manager.time_field]
        time_emb = self.real_time.forward(curr_ts, hist_timestamp)
        seq_emb += time_emb
        tar_time = self.real_time.forward(curr_ts, curr_ts.unsqueeze(-1)).squeeze(-2)
        seq_tar += tar_time
...
if __name__ == "__main__":
    ...
    settings_list = [
        ...
        genres_setting,
        SequenceSetting("movie_id_seq", element_setting=item_setting, group=history_group),
        SequenceSetting("genres_seq", element_setting=genres_setting, group=history_group),
    ]
