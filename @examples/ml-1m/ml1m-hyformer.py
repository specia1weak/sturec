...
class MovieLensConfig(ConfigBase):
    seed: int = 2025
    experiment_name: str = "ml1m-hyformer"
    ...
    append_aux_loss_weight: float = 0.0
...
class SpecialModel(BaseModel):
    def __init__(self, schema_manager: SchemaManager):
        ...
        self.append_query_branch = SequenceEvolutionBlock(self.seq_dim, cfg.num_heads, cfg.dropout_rate)
        ...
        self.append_query_head = MLP(
            cfg.num_queries * self.seq_dim,
            64,
            1,
            dropout_rate=cfg.dropout_rate,
        )
    ...
    def run_append_query_branch(
        self,
        seq_tokens: torch.Tensor,
        seq_len: torch.Tensor,
        curr_ts: torch.Tensor,
        query_seed_tokens: torch.Tensor,
    ) -> torch.Tensor:
        seq_padding_mask, _ = build_left_padded_mask(seq_len, seq_tokens.shape[1], seq_tokens.device)
        query_hist_ts = curr_ts.unsqueeze(1).expand(-1, query_seed_tokens.shape[1])
        query_time_emb = self.real_time(curr_ts, query_hist_ts)
        query_tokens = query_seed_tokens + query_time_emb
        ...
        branch_tokens = torch.cat([seq_tokens, query_tokens], dim=1)
        branch_padding_mask = torch.cat([seq_padding_mask, query_padding], dim=1)
        branch_output = self.append_query_branch(branch_tokens, branch_padding_mask)
        appended_output = branch_output[:, -query_tokens.shape[1] :, :].reshape(branch_output.shape[0], -1)
        return self.append_query_head(appended_output)
    ...
    def concat_embed_input_fields(self, interaction):
        ...
        return anon_seq, ns_tokens, share_emb, specific_emb, user_id_emb, item_id_emb, anon_seq_len, curr_ts

    def forward(self, anon_seq, ns_tokens, sha, spe, uid, iid, anon_seq_len, curr_ts):
        hyformer_output, query_seed, query_seed_tokens = self.hyformer(ns_tokens, anon_seq, anon_seq_len)
        append_query_logits = self.run_append_query_branch(
            anon_seq,
            anon_seq_len,
            curr_ts,
            query_seed_tokens,
        ...
        self._last_append_query_logits = append_query_logits
    ...
    def predict(self, interaction):
        anon_seq, ns_tokens, sha, spe, uid, iid, anon_seq_len, curr_ts = self.concat_embed_input_fields(interaction)
        return self.forward(anon_seq, ns_tokens, sha, spe, uid, iid, anon_seq_len, curr_ts)
    ...
    def calculate_loss(self, interaction):
        ...
        append_query_loss = cfg.append_aux_loss_weight * nn.functional.binary_cross_entropy_with_logits(
            self._last_append_query_logits.squeeze(-1), labels
        )
        loss = (
