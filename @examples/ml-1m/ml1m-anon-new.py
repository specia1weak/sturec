...
from betterbole.models.utils.transfomer import LearnablePositionalEmbedding, TransformerEncoder
...
class MovieLensConfig(ConfigBase):
    experiment_name: str = "ml1m-anon-new"
    dataset_name: str = "movie-lens-anon-new"
    emb_dim: int = 16
...
class ItemSeqEncoder(nn.Module):
    ...
    def _build_attn_mask(self, seq_len, max_len, device):
        ...
        attn_mask = key_padding_mask.unsqueeze(1).expand(-1, max_len, -1)
    ...
    def forward(self, seq, seq_len):
        ...
        positioned_seq = self.position_encoder.forward(seq)
        positioned_seq = positioned_seq * valid_mask.unsqueeze(-1)
        transformer_out = self.transformer_encoder.forward(positioned_seq, mask=attn_mask)
...
class SpecialModel(BaseModel):
    def __init__(self, schema_manager: SchemaManager):
        ...
        self.context_token_project = nn.Linear(self.whole_input_dim, self.seq_dim)
        self.seq_encoder = ItemSeqEncoder(self.seq_dim, max_len=20 + 1, causal=False)
    ...
    def build_anonymous_sequence(self, interaction):
        ...
        non_seq = self.omni_embedding.whole.forward(interaction)
        context_token = self.context_token_project(non_seq).unsqueeze(1)
        ...
        full_timestamp = torch.cat([hist_timestamp, curr_ts.unsqueeze(1)], dim=1)

        full_seq = torch.cat([hist_seq, context_token], dim=1)
        full_seq_len = seq_len + 1
        full_seq = full_seq + self.real_time(curr_ts, full_timestamp, seq_lens=full_seq_len)
        return full_seq, full_seq_len
    ...
    def concat_embed_input_fields(self, interaction):
        ...
        return anon_seq, share_emb, specific_emb, user_id_emb, item_id_emb, anon_seq_len

    def forward(self, anon_seq, sha, spe, uid, iid, anon_seq_len):
        ...
        _, seq_logits = self.seq_expert(self.seq_encoder.forward(anon_seq, anon_seq_len))
    ...
    def predict(self, interaction):
        anon_seq, sha, spe, uid, iid, anon_seq_len = self.concat_embed_input_fields(interaction)
        return self.forward(anon_seq, sha, spe, uid, iid, anon_seq_len)
...
if __name__ == "__main__":
    ...
    genres_setting = MultiSparseSetting("genres",FeatureSource.ITEM,embedding_dim=8,max_tag_len=8,min_freq=10,use_oov=True,)

    history_item_element = SparseEmbSetting("movie_id_seq_token",FeatureSource.SEQ,embedding_dim=16,min_freq=10,use_oov=True)
    history_genres_element = MultiSparseSetting("genres_seq_token",FeatureSource.SEQ,embedding_dim=8,max_tag_len=8,min_freq=10,use_oov=True)
