...
class MovieLensConfig(ConfigBase):
    experiment_name: str = "ml1m-anon-itemquery-new"
    dataset_name: str = "movie-lens-anon-itemquery-new"
    emb_dim: int = 16
...
class SpecialModel(BaseModel):
    def __init__(self, schema_manager: SchemaManager):
        ...
        self.item_query_project = nn.Linear(schema_manager.source2emb_dim(FeatureSource.ITEM_ID), self.seq_dim)
    ...
    def build_anonymous_sequence(self, interaction):
        ...
        item_query = self.omni_embedding.item_id.forward(interaction)
        item_query = self.item_query_project(item_query).unsqueeze(1)
        ...
        full_seq = torch.cat([hist_seq, item_query], dim=1)
...
if __name__ == "__main__":
    ...
    print("匿名序列 item-query 新架构编译成功，可供调用。")
