...
from betterbole.models.backbone.mmoe import SingleLayerMMoE
...
class SimplePLE(nn.Module):
    def __init__(self, schema_manager: SchemaManager):
        ...
        # self.ple = PLE(self.input_dim, num_domains, expert_dims=(32, 16), num_sha_experts=1, num_spe_experts=1)
        # self.ple = STAR(self.input_dim, num_domains)
        # self.ple = SharedBottomLess(self.input_dim, num_domains)
        self.ple = SingleLayerMMoE(self.input_dim, num_domains)
...
if __name__ == '__main__':
    ...
    auto_queue()
    device = "cuda"
    user_setting = SparseEmbSetting("user", FeatureSource.USER_ID, EMB_DIM, min_freq=5,use_oov=True) # 显存太低没法拉高dim
    item_setting = SparseEmbSetting("item", FeatureSource.ITEM_ID, EMB_DIM, min_freq=5, use_oov=True) # 显存太低没法拉高dim
    settings_list = [
        user_setting,
        item_setting,
        SparseEmbSetting("domain_indicator", FeatureSource.INTERACTION, 16, padding_zero=True, use_oov=False)
    ]
    manager = SchemaManager(settings_list, "diffmsr-amazonS-workdir", label_fields="label", domain_fields="domain_indicator", time_field="time")

    whole_lf = pl.scan_csv(DiffExperimentDataset.AMAZON_SMALL)
