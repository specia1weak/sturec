...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        offset = 0
        for idx, (fid, vs, dim) in enumerate(self._user_int_cols):
            col_name = f'user_int_feats_{fid}'
            ci = self._col_idx.get(col_name)
            remap_info = self._lookup_discrete_runtime(col_name, idx, vs)
            self._user_int_plan.append((ci, dim, offset, vs, remap_info))
            offset += dim

        self._item_int_plan = []
        offset = 0
        for idx, (fid, vs, dim) in enumerate(self._item_int_cols):
            col_name = f'item_int_feats_{fid}'
            ci = self._col_idx.get(col_name)
            remap_info = self._lookup_discrete_runtime(col_name, idx, vs)
            self._item_int_plan.append((ci, dim, offset, vs, remap_info))
            offset += dim
        ...
        for fid, dim in self._user_dense_cols:
            ...
            ci = self._col_idx.get(col_name)
            dense_cfg = self._dense_runtime_by_name.get(col_name, {})
            self._user_dense_plan.append((ci, dim, offset, dense_cfg))
            offset += dim
        ...
        for domain in self.seq_domains:
            ...
            for slot, fid in enumerate(sideinfo_fids):
                ...
                remap_info = self._lookup_discrete_runtime(col_name, slot, vs)
                side_plan.append((ci, slot, vs, remap_info))
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        self._runtime_discrete_by_name: Dict[str, Dict[str, Any]] = runtime_cfg.get('discrete', {})
        self._dense_runtime_by_name: Dict[str, Dict[str, Any]] = runtime_cfg.get('dense', {})
        self._keep_ids_by_name: Dict[str, np.ndarray] = {}
        oov_policy = runtime_cfg.get('oov_policy', {})
        keep_artifact = oov_policy.get('keep_ids_artifact') if isinstance(oov_policy, dict) else None
        if keep_artifact:
            keep_path = keep_artifact
            if not os.path.isabs(keep_path):
                keep_path = os.path.join(os.path.dirname(os.path.abspath(schema_path)), keep_path)
            with np.load(keep_path, allow_pickle=False) as npz:
                self._keep_ids_by_name = {name: npz[name] for name in npz.files}
            logging.info(
                f"Loaded runtime keep_ids artifact: {keep_path} "
                f"({len(self._keep_ids_by_name)} columns)"
            )
        ...
        for idx, (fid, vs, dim) in enumerate(self._user_int_cols):
            ...
            runtime_vs = self._runtime_vocab_for_col(f'user_int_feats_{fid}', idx, vs)
            self.user_int_vocab_sizes.extend([runtime_vs] * dim)
        ...
        for idx, (fid, vs, dim) in enumerate(self._item_int_cols):
            ...
            runtime_vs = self._runtime_vocab_for_col(f'item_int_feats_{fid}', idx, vs)
            self.item_int_vocab_sizes.extend([runtime_vs] * dim)
        ...
        for domain in self.seq_domains:
            ...
            self.seq_domain_vocab_sizes[domain] = [
                self._runtime_vocab_for_col(f"{cfg['prefix']}_{fid}", i, self.seq_vocab_sizes[domain][fid])
                for i, fid in enumerate(sideinfo)
            ]

            # max_len: from seq_max_lens arg; unspecified domains fall back to 256.
            self._seq_maxlen[domain] = seq_max_lens.get(domain, 256)

    def _runtime_vocab_for_col(
        self,
        col_name: str,
        fallback_index: int,
        raw_vocab_size: int,
    ) -> int:
        cfg = self._runtime_discrete_by_name.get(col_name)
        if not cfg:
            return raw_vocab_size
        compact_oov_idx = int(cfg.get('compact_oov_idx', raw_vocab_size))
        return compact_oov_idx

    def _lookup_discrete_runtime(
        self,
        col_name: str,
        fallback_index: int,
        vocab_size: int,
        ...
        if not cfg:
            return None
        keep_key = cfg.get('keep_ids_key', col_name)
        keep_ids = self._keep_ids_by_name.get(keep_key)
        compact_oov_idx = int(cfg.get('compact_oov_idx', vocab_size))
        return {
            'keep_ids': keep_ids,
            'compact_oov_idx': compact_oov_idx,
        }

    def _apply_compact_remap_inplace(
        self,
        arr: "npt.NDArray[np.int64]",
        remap_info: Optional[Dict[str, Any]],
    ) -> None:
        if remap_info is None:
            return
        keep_ids = remap_info.get('keep_ids')
        oov_idx = remap_info.get('compact_oov_idx')
        if keep_ids is None or oov_idx is None:
            return
        positive = arr > 0
        if not positive.any():
            return
        if keep_ids.size == 0:
            arr[positive] = oov_idx
            return
        vals = arr[positive]
        pos = np.searchsorted(keep_ids, vals)
        in_bounds = pos < keep_ids.size
        matches = np.zeros(vals.shape, dtype=bool)
        matches[in_bounds] = keep_ids[pos[in_bounds]] == vals[in_bounds]
        vals[matches] = pos[matches] + 1
        vals[~matches] = oov_idx
        arr[positive] = vals

    def _apply_dense_transform_inplace(
        self,
        ...
        if not dense_cfg:
            return
        transform = dense_cfg.get('transform', 'none')
        if transform == 'none':
            return
        if transform == 'log1p':
        ...
        if transform == 'clip_log1p':
            clip_value = dense_cfg.get('clip_value')
            if clip_value is None:
                np.maximum(arr, 0.0, out=arr)
            else:
                np.clip(arr, 0.0, float(clip_value), out=arr)
            arr[:] = np.log1p(arr)
            return
        raise ValueError(f"Unsupported dense transform: {transform}")
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        user_int[:] = 0
        for ci, dim, offset, vs, remap_info in self._user_int_plan:
            col = batch.column(ci)
            if dim == 1:
                ...
                if vs > 0:
                    ...
                    arr[:] = 0
                self._apply_compact_remap_inplace(arr, remap_info)
                user_int[:, offset] = arr
            else:
                ...
                if vs > 0:
                    ...
                    padded[:] = 0
                self._apply_compact_remap_inplace(padded, remap_info)
                user_int[:, offset:offset + dim] = padded

        # ---- item_int ----
        item_int = self._buf_item_int[:B]
        item_int[:] = 0
        for ci, dim, offset, vs, remap_info in self._item_int_plan:
            col = batch.column(ci)
            if dim == 1:
                arr = col.fill_null(0).to_numpy(zero_copy_only=False).astype(np.int64)
                arr[arr <= 0] = 0
        ...
        user_dense[:] = 0
        for ci, dim, offset, dense_cfg in self._user_dense_plan:
            col = batch.column(ci)
            padded = self._pad_varlen_float_column(col, dim, B)
            self._apply_dense_transform_inplace(padded, dense_cfg)
        ...
        for domain in self.seq_domains:
            ...
            col_data = []
            for ci, slot, vs, remap_info in side_plan:
                col = batch.column(ci)
                col_data.append((col.offsets.to_numpy(), col.values.to_numpy(), vs, ci, remap_info))

            for c, (offs, vals, vs, ci, remap_info) in enumerate(col_data):
                for i in range(B):
                    s = int(offs[i])
                    e = int(offs[i + 1])
                    rl = e - s
            ...
            for c, (_, _, vs, ci, remap_info) in enumerate(col_data):
                slice_c = out[:, c, :]
                if vs > 0:
                    self._record_oob(f'seq_{domain}', ci, slice_c, vs)
                else:
                    slice_c[:] = 0
                self._apply_compact_remap_inplace(slice_c, remap_info)
