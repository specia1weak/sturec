...
$logFile = Join-Path -Path $logDir -ChildPath "agent2_status.log"
$latestFile = Join-Path -Path $logDir -ChildPath "agent2_latest_status.txt"
$snapshotFile = Join-Path -Path $PSScriptRoot -ChildPath ".agent2_snapshot.json"
...
if (Test-Path -LiteralPath $snapshotFile) {
    try {
        $previous = Get-Content -LiteralPath $snapshotFile -Raw -Encoding UTF8 | ConvertFrom-Json -AsHashtable
    } catch {
        $previous = @{}
...
Write-LogLine "=== Agent2 Monitor STARTED (interval: ${IntervalSeconds}s, maxIterations: ${MaxIterations}) ==="
...
while ($true) {
    ...
    foreach ($d in $dirs) {
        ...
        foreach ($name in $common) {
            ...
            if (($prevRow["LastWrite"] -ne $currRow["LastWrite"]) -or ($prevRow["Length"] -ne $currRow["Length"])) {
                ...
                if ($d -eq "2_Doing") {
                    $alerts.Add("WORK IN PROGRESS UPDATED -> $name")
                }
            }
        }
    }
    ...
    foreach ($name in @($addedByName.Keys | Sort-Object)) {
        ...
        if ($targetDir -eq "1_Todo") {
            $alerts.Add("NEW TASK READY -> $name")
        } elseif ($targetDir -eq "2_Doing") {
            $alerts.Add("TASK CLAIMED -> $name")
        } elseif ($targetDir -eq "3_Done") {
            $alerts.Add("TASK COMPLETED -> $name")
        } elseif ($targetDir -eq "4_Block") {
            $alerts.Add("TASK BLOCKED -> $name")
        }
    }
    ...
    if ($initializeBaseline) {
        ...
        Write-LatestStatus -Summary $summary -Changes @() -Alerts @()
    ...
    Write-LatestStatus -Summary $summary -Changes @($changes) -Alerts @($alerts)
    $current | ConvertTo-Json -Compress | Set-Content -LiteralPath $snapshotFile -Force -Encoding UTF8
    $previous = $current
    $iteration += 1
...
Write-LogLine "=== Agent2 Monitor FINISHED ==="
