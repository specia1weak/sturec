...
from betterbole.utils.observatory.metrics import per_domain_code_usage
from betterbole.utils.observatory.plots import plot_dim_profile, plot_domain_code_usage, plot_multi_series, plot_scalar_series, plot_step_dim_heatmap, PlotSeries
...
class VQShareModel(MSRModel):
    def __init__(
        ...
        if self.input_dim <= 0:
            raise ValueError("VQShareModel requires non-empty input embeddings.")
    ...
    def _observatory_output_dir(self, ctx: TrainContext) -> Path:
        ...
        if model_name:
            ...
            output_name = str(getattr(ctx.cfg, "experiment_name", "vq_share"))
        ...
        recorder.register(
            ...
            TensorMonitorOptions(
                display=TensorDisplayConfig(
                    ...
                    show_per_dim=True,
                    max_display_dims=16,
                    topk_display_dims=12,
                    rank_by="variance",
                )
            ),
        )
        for name in ["vq_shared_logits", "vq_specific_logits", "vq_quantized_cos", "vq_residual_norm"]:
            recorder.register(
                name,
                TensorMonitorOptions(
                    display=TensorDisplayConfig(
        ...
        if hasattr(recorder, "configure_relations"):
            recorder.configure_relations(
                RelationOptions(
                    ...
                    rank=6,
                    max_pairs=6,
                    names=("vq_shared_hidden", "vq_specific_feature_fluctuation"),
                )
            )
    ...
    def _append_observatory_scalars(self, step: int) -> None:
        ...
        scalar_values = {
            "code_usage_entropy": float(debug["code_usage_entropy"]),
            "used_code_ratio": float(debug["used_code_ratio"]),
            "shared_feature_var": float(contrib["shared_feature_var"]),
            "specific_feature_var": float(contrib["specific_feature_var"]),
            "shared_logit_var": float(contrib["shared_logit_var"]),
            "specific_logit_var": float(contrib["specific_logit_var"]),
            "quantized_cos_mean": float(self._latest_debug["quantized_cos"].float().mean().item()),
            "residual_norm_mean": float(self._latest_debug["residual_norm"].float().mean().item()),
        }
    ...
    def _export_observatory_artifacts(self, ctx: TrainContext) -> None:
        ...
        if self._observatory_steps:
            plot_multi_series(
                [
                    PlotSeries("shared_feature_var", self._observatory_steps, self._observatory_scalar_history.get("shared_feature_var", [])),
                    PlotSeries("specific_feature_var", self._observatory_steps, self._observatory_scalar_history.get("specific_feature_var", [])),
                ],
                title="vq_feature_var_series",
                xlabel="step",
                ylabel="feature_var",
                save_path=output_dir / "vq_feature_var_series.png",
            )
            plot_multi_series(
                [
                    PlotSeries("shared_logit_var", self._observatory_steps, self._observatory_scalar_history.get("shared_logit_var", [])),
                    PlotSeries("specific_logit_var", self._observatory_steps, self._observatory_scalar_history.get("specific_logit_var", [])),
                ],
                title="vq_logit_var_series",
                xlabel="step",
                ylabel="logit_var",
                save_path=output_dir / "vq_logit_var_series.png",
            )
            plot_multi_series(
                [
                    PlotSeries("code_usage_entropy", self._observatory_steps, self._observatory_scalar_history.get("code_usage_entropy", [])),
                    PlotSeries("used_code_ratio", self._observatory_steps, self._observatory_scalar_history.get("used_code_ratio", [])),
                ],
                title="vq_codebook_series",
                xlabel="step",
                ylabel="value",
                save_path=output_dir / "vq_codebook_series.png",
            )
            plot_multi_series(
                [
                    PlotSeries("quantized_cos_mean", self._observatory_steps, self._observatory_scalar_history.get("quantized_cos_mean", [])),
                    PlotSeries("residual_norm_mean", self._observatory_steps, self._observatory_scalar_history.get("residual_norm_mean", [])),
                ],
                title="vq_alignment_series",
                xlabel="step",
                ylabel="value",
                save_path=output_dir / "vq_alignment_series.png",
            )

        _, shared_batch_var_history = recorder.get_vector_history("vq_shared_hidden", "batch_var")
        if shared_batch_var_history:
            latest_shared_var = shared_batch_var_history[-1]
            plot_dim_profile(
                dim_idx=list(range(len(latest_shared_var))),
                values=latest_shared_var,
                title="vq_shared_hidden_batch_var",
                ylabel="batch_var",
                save_path=output_dir / "vq_shared_hidden_batch_var.png",
            )
        _, specific_batch_var_history = recorder.get_vector_history("vq_specific_feature_fluctuation", "batch_var")
        if specific_batch_var_history:
            latest_specific_var = specific_batch_var_history[-1]
            plot_dim_profile(
                dim_idx=list(range(len(latest_specific_var))),
                values=latest_specific_var,
                title="vq_specific_fluctuation_batch_var",
                ylabel="batch_var",
                save_path=output_dir / "vq_specific_fluctuation_batch_var.png",
            )

        shared_steps, shared_step_dim = recorder.get_step_dim_matrix("vq_shared_hidden", "batch_var")
        if shared_step_dim.size > 0:
            plot_step_dim_heatmap(
                values=shared_step_dim,
                title="vq_shared_hidden_step_dim_var",
                save_path=output_dir / "vq_shared_hidden_step_dim_var.png",
            )
        specific_steps, specific_step_dim = recorder.get_step_dim_matrix("vq_specific_feature_fluctuation", "batch_var")
        if specific_step_dim.size > 0:
            plot_step_dim_heatmap(
                values=specific_step_dim,
                title="vq_specific_fluctuation_step_dim_var",
                save_path=output_dir / "vq_specific_fluctuation_step_dim_var.png",
            )

        domain_code = per_domain_code_usage(
            self._latest_debug["code_indices"],
            self._latest_debug["domain_ids"],
            self.codebook_size,
        )
        if domain_code.numel() > 0:
            plot_domain_code_usage(
                values=domain_code.detach().cpu().numpy(),
                title="vq_domain_code_usage",
                save_path=output_dir / "vq_domain_code_usage.png",
            )
    ...
    def _record_debug_tensors(self, recorder, step: Optional[int] = None) -> None:
        ...
        recorder.record("vq_code_usage", code_hist, step=step)
        recorder.record("vq_shared_hidden", self._latest_debug["shared_hidden"], step=step)
        recorder.record("vq_shared_feature_importance", self._latest_debug["shared_feature_importance"], step=step)
        recorder.record("vq_shared_logits", self._latest_debug["shared_logits"].unsqueeze(-1), step=step)
        recorder.record("vq_specific_logits_raw", self._latest_debug["specific_logits_raw"].unsqueeze(-1), step=step)
        recorder.record("vq_specific_logits", self._latest_debug["specific_logits"].unsqueeze(-1), step=step)
        recorder.record("vq_specific_hidden", self._latest_debug["specific_hidden"], step=step)
        recorder.record("vq_specific_feature_bias", self._latest_debug["specific_feature_bias"], step=step)
        recorder.record("vq_specific_feature_fluctuation", self._latest_debug["specific_feature_fluctuation"], step=step)
        recorder.record("vq_specific_feature_importance", self._latest_debug["specific_feature_importance"], step=step)
        recorder.record("vq_shared_norm", self._latest_debug["shared_norm"].unsqueeze(-1), step=step)
        recorder.record("vq_residual_norm", self._latest_debug["residual_norm"].unsqueeze(-1), step=step)
        recorder.record("vq_quantized_cos", self._latest_debug["quantized_cos"].unsqueeze(-1), step=step)
    ...
    def on_train_epoch_start(self, ctx: TrainContext) -> None:
        if ctx.recorder is not None:
            self._setup_observatory(ctx.recorder)
