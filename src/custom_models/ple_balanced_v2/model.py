...
class PLEBalancedV2Encoder(nn.Module):
    def __init__(
            self,
            input_dim: int,
            num_domains: int,
...
class PLEBalancedV2Model(MSRModel):
    def __init__(
            self,
            ...
            domain_adv_weight: float = 0.05,
            domain_adv_lambda: float = 1.0,
            domain_adv_hidden_dims: Iterable[int] = (64,),
            domain_adv_dropout_rate: float = 0.0,
    ):
        ...
        self.domain_adv_lambda = float(domain_adv_lambda)

        self.encoder = PLEBalancedV2Encoder(
        ...
        self.shared_readout_norm = nn.LayerNorm(self.encoder.output_dim, elementwise_affine=False)
        self.shared_head = nn.Linear(self.encoder.output_dim, 1)
        adv_hidden_dims = to_dims(domain_adv_hidden_dims, (max(1, self.encoder.output_dim // 2),))
        self.domain_discriminator = MLP(
            self.encoder.output_dim,
            *adv_hidden_dims,
            self.num_domains,
            dropout_rate=domain_adv_dropout_rate,
            activation=activation,
            batch_norm=batch_norm,
        )
        ...
        self._latest_domain_adv_loss = torch.tensor(0.0)
        self._latest_debug = {}
    ...
    def _forward_dict(self, x: torch.Tensor, domain_ids: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        domain_logits = self.domain_discriminator(gradient_reversal(shared_hidden, self.domain_adv_lambda))
        return {
    ...
    def _balanced_domain_ce(
        ...
        if loss_list:
            ...
            balanced_loss = domain_logits.new_zeros(())
        return balanced_loss, per_domain_losses, present_mask
    ...
    def calculate_loss(self, interaction):
        ...
        shared_aux_loss, shared_domain_losses, shared_present_mask = self._balanced_shared_bce(
            forward_dict["shared_logits"],
            labels,
            domain_ids,
        )
        domain_adv_loss, domain_adv_domain_losses, domain_adv_present_mask = self._balanced_domain_ce(
            forward_dict["domain_logits"],
            domain_ids,
        )
        total_loss = (
            task_loss
            + self.shared_aux_weight * shared_aux_loss
            + self.domain_adv_weight * domain_adv_loss
        )

        self._latest_task_loss = task_loss.detach()
        self._latest_shared_aux_loss = shared_aux_loss.detach()
        self._latest_domain_adv_loss = domain_adv_loss.detach()
        self._latest_debug = {
            ...
            "domain_logits": forward_dict["domain_logits"].detach(),
            ...
            "domain_adv_domain_losses": domain_adv_domain_losses.detach(),
            "domain_adv_present_mask": domain_adv_present_mask.detach(),
        }
        return total_loss
    ...
    def debug_state(self) -> Dict[str, float]:
        if not self._latest_debug:
            return {
                ...
                "domain_adv_present_domains": 0.0,
            }
        shared_present_mask = self._latest_debug["shared_present_mask"]
        domain_adv_present_mask = self._latest_debug["domain_adv_present_mask"]
        return {
            "task_loss": float(self._latest_task_loss.item()),
            "shared_aux_loss": float(self._latest_shared_aux_loss.item()),
            "domain_adv_loss": float(self._latest_domain_adv_loss.item()),
            "shared_present_domains": float(shared_present_mask.sum().item()),
            "domain_adv_present_domains": float(domain_adv_present_mask.sum().item()),
        }
    ...
    def contribution_state(self) -> Dict[str, float]:
        if not self._latest_debug:
            return {
                ...
                "domain_adv_loss_gap": 0.0,
                "shared_domain_loss_mean": 0.0,
                "domain_adv_loss_mean": 0.0,
                "shared_raw_abs_mean": 0.0,
                "shared_abs_mean": 0.0,
                ...
                "domain_accuracy": 0.0,
                "domain_entropy": 0.0,
                "shared_domain_mean_dispersion": 0.0,
            }

        shared_hidden_raw = self._latest_debug["shared_hidden_raw"].float()
        ...
        domain_logits = self._latest_debug["domain_logits"].float()
        domain_ids = self._latest_debug["domain_ids"].long()
        shared_domain_losses = self._latest_debug["shared_domain_losses"].float()
        domain_adv_domain_losses = self._latest_debug["domain_adv_domain_losses"].float()
        shared_present_mask = self._latest_debug["shared_present_mask"]
        domain_adv_present_mask = self._latest_debug["domain_adv_present_mask"]

        shared_raw_feature_var = float(shared_hidden_raw.var(unbiased=False).item())
        ...
        if shared_present_mask.any():
            active_losses = shared_domain_losses[shared_present_mask]
        ...
        if domain_adv_present_mask.any():
            active_adv_losses = domain_adv_domain_losses[domain_adv_present_mask]
            domain_adv_loss_gap = float((active_adv_losses.max() - active_adv_losses.min()).item())
            domain_adv_loss_mean = float(active_adv_losses.mean().item())
        else:
            domain_adv_loss_gap = 0.0
            domain_adv_loss_mean = 0.0

        if domain_logits.numel() > 0:
            domain_pred = domain_logits.argmax(dim=-1)
            domain_accuracy = float((domain_pred == domain_ids).float().mean().item())
            domain_probs = torch.softmax(domain_logits, dim=-1)
            domain_entropy = float((-domain_probs * torch.log(domain_probs.clamp_min(1e-12))).sum(dim=-1).mean().item())
        else:
            domain_accuracy = 0.0
            domain_entropy = 0.0
        ...
        for domain_idx in range(self.num_domains):
            mask = domain_ids == domain_idx
            if mask.any():
                shared_domain_means.append(shared_hidden[mask].mean(dim=0))
        if len(shared_domain_means) >= 2:
            shared_domain_mean_dispersion = float(torch.stack(shared_domain_means, dim=0).var(dim=0, unbiased=False).mean().item())
        else:
            shared_domain_mean_dispersion = 0.0

        return {
            "shared_feature_var": shared_feature_var,
            "shared_raw_feature_var": shared_raw_feature_var,
            ...
            "domain_adv_loss_gap": domain_adv_loss_gap,
            "shared_domain_loss_mean": shared_domain_loss_mean,
            "domain_adv_loss_mean": domain_adv_loss_mean,
            "shared_raw_abs_mean": float(shared_hidden_raw.abs().mean().item()),
            ...
            "shared_domain_mean_dispersion": shared_domain_mean_dispersion,
        }
    ...
    def _record_debug_tensors(self, recorder) -> None:
        ...
        recorder.record("ple_balanced_v2_shared_hidden", self._latest_debug["shared_hidden"])
        recorder.record("ple_balanced_v2_shared_hidden_raw", self._latest_debug["shared_hidden_raw"])
        recorder.record("ple_balanced_v2_specific_hidden", self._latest_debug["specific_hidden"])
        recorder.record("ple_balanced_v2_shared_logits", self._latest_debug["shared_logits"].unsqueeze(-1))
        recorder.record("ple_balanced_v2_specific_logits", self._latest_debug["specific_logits"].unsqueeze(-1))
        recorder.record("ple_balanced_v2_domain_logits", self._latest_debug["domain_logits"])
        recorder.record("ple_balanced_v2_shared_domain_losses", self._latest_debug["shared_domain_losses"].unsqueeze(-1))
        recorder.record("ple_balanced_v2_domain_adv_domain_losses", self._latest_debug["domain_adv_domain_losses"].unsqueeze(-1))
    ...
    def on_eval_epoch_end(self, metrics: Optional[Dict], ctx: TrainContext) -> None:
        ...
        print(
            "[PLE-Balanced-v2 Debug] "
            f"task={debug['task_loss']:.5f} "
            f"shared_aux={debug['shared_aux_loss']:.5f} "
            f"domain_adv={debug['domain_adv_loss']:.5f} "
            f"present(shared/adv)={int(debug['shared_present_domains'])}/{int(debug['domain_adv_present_domains'])} "
            ...
            f"adv_gap={contrib['domain_adv_loss_gap']:.5f} "
            f"domain_acc={contrib['domain_accuracy']:.3f} "
            f"domain_entropy={contrib['domain_entropy']:.3f} "
            f"shared_disp={contrib['shared_domain_mean_dispersion']:.5f} "
            f"abs_mean(raw/shared/specific)={contrib['shared_raw_abs_mean']:.4f}/{contrib['shared_abs_mean']:.4f}/{contrib['specific_abs_mean']:.4f} "
