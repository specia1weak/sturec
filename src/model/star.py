...
class STAR(nn.Module):
    def __init__(self, emb_size, num_domains):
        super().__init__()
        linear_stream = [emb_size, emb_size // 2, emb_size, emb_size // 2, emb_size,]
        ...
        self.towers = MultiScenarioContainer(num_domains, ModuleFactory.build_tower(emb_size))
...
class StarPle(nn.Module):
    def __init__(self, emb_size, num_domains):
        super().__init__()
        linear_stream = [emb_size, emb_size // 2, emb_size]
        ## STAR部分
        self.spe_experts = nn.ModuleList([StarExpert(*linear_stream) for _ in range(num_domains)])
        self.sha_expert = StarExpert(*linear_stream)

        ## PLE部分
        self.shared_gate_layer1 = ModuleFactory.build_gate(emb_size, num_domains)()

        self.multi_experts_layer2 = MultiScenarioContainer(num_domains, ModuleFactory.build_expert(emb_size))
        self.shared_expert_layer2 = MLP(emb_size, emb_size // 2, emb_size)
        self.multi_gates_layer2 = MultiScenarioContainer(num_domains, ModuleFactory.build_gate(emb_size))
        self.towers = MultiScenarioContainer(num_domains, ModuleFactory.build_tower(emb_size))
