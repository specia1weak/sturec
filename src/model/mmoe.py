...
class SingleLayerMTLMMoE(nn.Module):
    def __init__(self, emb_size, task_names, expert_dim=64):
        super().__init__()
        ...
        build_silu_expert = lambda: MLP(emb_size, emb_size // 2, expert_dim, activation="silu", batch_norm=True)
        self.layer1_experts = MSC(num_experts, build_silu_expert)
        self.layer1_gates = MTC(task_names, MF.build_gate(emb_size, num_experts))
        self.towers = MTC(task_names, MF.build_tower(emb_size // 2))
